...
describes what users get. Pick a format with `-o`, or let noaa-tides choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
noaa-tides <command> -o table   # aligned columns for reading
noaa-tides <command> -o jsonl   # one JSON object per line, for piping
noaa-tides <command> -o json    # a single JSON array
noaa-tides <command> -o csv     # spreadsheet friendly
noaa-tides <command> -o tsv     # tab-separated
noaa-tides <command> -o url     # just the URL column
noaa-tides <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
noaa-tides <command> --fields id,title,url
```
...
```bash
noaa-tides <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
noaa-tides <command>            # a table, because this is a terminal
noaa-tides <command> | wc -l    # JSONL, because this is a pipe
```
